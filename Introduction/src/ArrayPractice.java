...
public class ArrayPractice{
    ...
    public static int[] merge(int[] a, int[] b){
        int len_a = a.length;
        int len_b = b.length;
        int[] mergeArr = new int[(len_a + len_b)];
        for (int i = 0; i<len_a; i++){
            mergeArr[i] = a[i];
        }
        for (int i = 0; i<len_b; i++){
            mergeArr[len_a+i] = b[i];
        }
        int temp;
        /*start from the first element as the smallest,
         in inner loop, if the element is smaller, set the index to nextSmallest,
         then swap the position*/
        for (int i=0; i<mergeArr.length-1; i++){

            int nextSmallest = i;
            for  ( int j=i+1; j<mergeArr.length; j++){
                if (mergeArr[j] < mergeArr[nextSmallest]){
                    nextSmallest = j;
                }
            }
            temp = mergeArr[i];
            mergeArr[i] = mergeArr[nextSmallest];
            mergeArr[nextSmallest] = temp;
        }
        return mergeArr;
    }
    ...
    public static int longestIncreasingSequence(int[] a){
        ...
        for (int i=0; i<(a.length-1); i++){
            if (a[i+1] > a[i]){
                ...
                /*when the next element is not bigger than the front, set length to default,
                if length > longest_seq, set longest_seq = length */
